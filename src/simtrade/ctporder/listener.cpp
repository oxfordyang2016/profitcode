...
#include "simtrade/ctporder/message_sender.h"
#include "simtrade/ctporder/listener.h"
...
void Listener::OnRtnOrder(CThostFtdcOrderField* order) {
  ...
  int orderref = t_m->GetOrderRef(ctp_order_ref);
  if (orderref == -1) {
    printf("ctporderref %d not found\n", ctp_order_ref);
    return;
  }
  printf("map it into %d\n", orderref);
  exchangeinfo.order_ref = orderref;
...
void Listener::OnRtnTrade(CThostFtdcTradeField* trade) {
  ...
  exchangeinfo.order_ref = orderref;
  printf("received onRtnTrade for %d, and map it into %d\n", ctp_order_ref, orderref);
...
void Listener::OnRspQryInvestorPosition(CThostFtdcInvestorPositionField* investor_position,
  ...
  if (investor_position) {
    ...
    if (result) {
      ...
      info.trade_price = investor_position->PositionCost/investor_position->Position;
