...
struct Order {
  ...
  void Show(FILE* stream) const {
    ...
    fprintf(stream, "%ld %06ld Order %s |",
      ...
      fprintf(stream, " %lf@%d %d %s %d %s %s %s\n", price, size, traded_size, OrderSide::ToString(side), order_ref, OrderAction::ToString(action), OrderStatus::ToString(status), Offset::ToString(offset));
  }
};
